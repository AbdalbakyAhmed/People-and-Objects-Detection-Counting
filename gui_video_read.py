    ...
    def get_frame(self):
        if self.vid.isOpened():
            ...
            if ret:
                ...
                #         and (int(dict_objects_occurrence['person']) >=
                #              self.dict_detection_activate_config['person_thresh']) \
                #         and (self.safe_flag is False)\
                #         and self.dict_detection_activate_config['person']:
                #     alarm_trigger(1)
                #     self.counter_start = chk_counter  # update old value
                # else:
                #     pass
                # if (temp_chk > counter_thresh) \
                #         and (int(dict_objects_occurrence['person']) >=
                #              self.dict_detection_activate_config['person_thresh']) \
                #         and (self.safe_flag is True)\
                #         and self.dict_detection_activate_config['person']:
                #     alarm_trigger(2)
                #     self.counter_start = chk_counter  # update old value
                # else:
