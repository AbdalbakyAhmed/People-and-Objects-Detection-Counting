...
#     if val > counter_thresh:
#         win32api.Beep(570, 200)
#     else:
#         pass
mixer.init()
...
def main():
    ...
    safe_flag = False
        while True:
            ...
            temp_chk = chk_counter - counter_start
            if (temp_chk > counter_thresh) \
                    and (int(dict_objects_occurrence['person']) >= 1) \
                    and (safe_flag is False):
                alarm_trigger(1)
                counter_start = chk_counter  # update old value
            else:
                pass
            if (temp_chk > counter_thresh) \
                    and (int(dict_objects_occurrence['person']) >= 1) \
                    and (safe_flag is True):
                alarm_trigger(2)
                counter_start = chk_counter  # update old value
            else:
                pass
            # if (int(dict_objects_occurrence['person']) > 1) and ( chk_counter - counter_start > counter_thresh):
            # #     os.system("real-police-siren-ringtone.mp3")
            #     win32api.Beep(570, 500)
            #     counter_start = chk_counter #update old value
            ##
            ...
            if key == ord("d"):
                safe_flag = False
                mixer.music.stop()
                print("Alarm System is initiated again")
                time.sleep(0.2)
            ##
            th.join()
            ##
